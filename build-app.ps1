...
#
# Uso: .\build-app.ps1
# Saida: dist-electron\ZeroInsight Setup *.exe
...
# Pre-verificacoes
Write-Host ""
Write-Host "[pre] Verificando pre-requisitos..." -ForegroundColor Cyan
...
$nodeOk = $null
try { $nodeOk = node --version 2>&1 } catch {}
...
Write-Host "   OK - dist/backend/backend.exe gerado ($sizeMB MB)" -ForegroundColor Green
...
Write-Host "=============================" -ForegroundColor Magenta
Write-Host ""
Write-Host "  Instalador: $($installer.FullName)" -ForegroundColor Yellow
Write-Host ""
